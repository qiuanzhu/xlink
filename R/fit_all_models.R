#' Model fitting results for each SNP considering as XCI, XCI-E and XCI-S tpye
#' 
#' \code{fit_all_models} returns model fitting results for each SNP undertanding as XCI, XCI-E and XCI-S tpye resectively. Model comparison results is provided by using AIC as a criterion.       
#' 
...
fit_all_models <- function(resp, os, ostime, snp, gender, male, female, covars, model, data) {
    ...
    if (model == "survival") {
        ...
        model_ch <- c("XCI-S")
        model_ind <- ((LR_a - 2) > (LR_s - 3) && (LR_a > LR_g)) + 2 * ((LR_g - 2) > (LR_s - 3) && (LR_g > LR_a)) + 
            3 * ((LR_a == LR_g) && (LR_g - 2) > (LR_s - 3))
