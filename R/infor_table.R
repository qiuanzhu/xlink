...
infor_table <- function(x, snp, covar_n, model) {
    ...
    if (model == "survival") {
        ...
        Table <- cbind(hz, as.data.frame(CI), pv)
        rownames(Table) <- covar_n
        colnames(Table) <- c("Hazard Ratio", "Confidence Interval (95%)", "P Value")
        return(Table)
    }
    ...
    if (model %in% c("binary", "linear")) {
        ...
        pv <- x[, 4]
        CI <- paste("[", CI_low, ",", CI_up, "]", sep = "")
        Table <- cbind(es, as.data.frame(CI), pv)
        rownames(Table) <- covar_n
        colnames(Table) <- c("Estimate", "Confidence Interval (95%)", "P Value")
        return(Table)
    }
